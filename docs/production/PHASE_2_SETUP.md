...
> account, so **you** perform it in the dashboard; then paste the marked values
> back to me (📋 **PASTE BACK**) and I wire them into config and verify.
>
...
Goal: a production DB fully isolated from the pilot. Recommended: an **empty
`production` branch** + apply our schema (a clean dataset, no pilot data copied).

1. Neon Console → Novara project → **Branches** → **New branch**.
   - Name: `production`. Create it **empty** (or branch then truncate — empty is cleaner).
...
3. Copy the **pooled** connection string — the host must contain `-pooler.neon.tech`
   and end with `?sslmode=require`.
4. Apply the schema (run locally with the prod URL):
   ```bash
   ...
   psql "$PROD_DATABASE_URL" -c "SELECT count(*) FROM contacts;"   # expect 0
   ```
5. Enable PITR / history retention on the `production` branch before real users.

📋 **PASTE BACK:** the pooled `DATABASE_URL` (I'll treat it as a secret — it goes
only into Render, never into git). *Or* just confirm "schema applied, 4 tables, 0 rows."
...
- If that instance is `pk_test_`, create/enable the **Production** instance for this
  app instead (Clerk requires `pk_live_`/`sk_live_` + a custom domain for real users).
  **Never promote the pilot dev instance.**

Then, in the production instance:
1. **Domains** → set the Clerk custom domain (Clerk gives CNAME records, typically
   on `clerk.novaraconnect.group`). Add them at your registrar and let Clerk verify.
2. **Allowed origins / redirect URLs** — add all of:
...
3. **Sign-in methods**: enable what you want for production (email/password, OAuth…).
   If you enable OAuth, note it needs extra handling inside the iOS webview (Phase 4).
4. Copy the keys from **API Keys**.

📋 **PASTE BACK:**
- `pk_live_…` (publishable — not secret; goes to both backend `CLERK_PUBLISHABLE_KEY`
  and frontend `VITE_CLERK_PUBLISHABLE_KEY`)
- `sk_live_…` (secret — backend `CLERK_SECRET_KEY` only)
- Confirm the Clerk custom domain hostname you set.
...
**Environment variables** (Environment tab). Secrets marked 🔒:
...
Generate the VAPID keypair (run locally, paste both values into Render):
```bash
...
📋 **PASTE BACK:** the Render backend URL (e.g. `https://novara-prod-api.onrender.com`)
and the `/api/healthz` JSON.
...
📋 **PASTE BACK:** the Render static-site URL and whether the build succeeded.
...
📋 **PASTE BACK:** "both domains verified" once TLS is green.
